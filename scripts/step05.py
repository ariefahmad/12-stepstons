...
class lin_2dconv():
    ...
    def __init__(self):
        ...
        self.grid_points_x = 81
        self.grid_points_y = 81
    ...
    def disc_2d(self):
        """
        Applying 2d discretisation; for linear convection;
        use vector processes instead of nested for loops when dealing with 2d
        """
        for n in range(0, self.nt + 1):
            un = self.u.copy()
            self.u[1:, 1:] = (un[1:, 1:] - (self.c * (self.dt / self.dx) * (un[1:, 1:] - un[1:, :-1])) - (self.c * (self.dt / self.dx) * (un[1:, 1:] - un[:-1, 1:])))
...
if __name__ == '__main__':
    ...
    ax.set_ylabel("$y$")
    ax.set_zlabel("$u$")
    ax.text2D(0.35, 0.95, "2-D Linear Convection", transform=ax.transAxes)
    plt.show()
